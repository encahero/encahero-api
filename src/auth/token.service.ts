...
export class TokenService {
    ...
    async generateAccessToken(userId: string): Promise<string> {
        return this.jwtService.sign({ userId }, { expiresIn: '15m', secret: this.configService.get('SECRET_KEY') });
    }

    async generateRefreshToken(userId: string): Promise<string> {
        return this.jwtService.sign({ userId }, { expiresIn: '7d', secret: this.configService.get('REFRESH_KEY') });
    }
