...
export class QuizController {
    ...
    async randomQuizFromCollection(
        ...
        @Query('mixed', new DefaultValuePipe('false')) mixedStr: string,
    ) {
        const isMixed = mixedStr === 'true';
        const data = await this.quizService.randomQuizFromCollection(collectionId, userId, isMixed, limit);
    ...
    async answerQuiz(
        ...
        const data = await this.quizService.answerQuiz(collectionId, cardId, userId, timeZone, answer);
